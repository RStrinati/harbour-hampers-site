...
function formatEmailContent(data) {
  return {
    subject: `New HAMPA Enquiry from ${data.name}`,
    html: `
      <h2>New HAMPA Website Enquiry</h2>
      <p><strong>Name:</strong> ${data.name}</p>
      <p><strong>Email:</strong> ${data.email}</p>
      <p><strong>Phone:</strong> ${data.phone || 'Not provided'}</p>
      <p><strong>Property Type:</strong> ${data['property-type'] || 'Not specified'}</p>
      <p><strong>Property Address:</strong> ${data.address || 'Not provided'}</p>
      <p><strong>Guests Per Month:</strong> ${data['guests-per-month'] || 'Not specified'}</p>
...
Guests Per Month: ${data['guests-per-month'] || 'Not specified'}
...
async function sendEmail(content, formData) {
  // Option 1: Use Resend API (popular, simple)
  const RESEND_API_KEY = 'YOUR_RESEND_API_KEY'; // Set this as environment variable
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'noreply@hampa.com.au',
        to: ['hello@hampa.com.au'],
    ...
    // Fallback: Use webhook (Zapier, Make.com, etc.)
    return await sendViaWebhook(content, formData);
  }
}
